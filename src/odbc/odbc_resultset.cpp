...
#include "include/odbc/odbc_resultset_metadata.h" 
...
// ctor
wxOdbcResultSet::wxOdbcResultSet(wxOdbcInterface* pInterface): wxDatabaseResultSet()
{
...
// dtor
wxOdbcResultSet::~wxOdbcResultSet()
{
    Close();
...
int wxOdbcResultSet::GetResultInt(int nField)
{
    if ( m_fieldValues[nField-1].IsNull() )
    {
...
wxString wxOdbcResultSet::GetResultString(int nField)
{
    if ( m_fieldValues[nField-1].IsNull() )
    {
        if ( GetFieldLength(nField) < 0 )
...
long wxOdbcResultSet::GetResultLong(int nField)
{
    if ( m_fieldValues[nField-1].IsNull() )
    {
        if ( GetFieldLength(nField) < 0 )
...
bool wxOdbcResultSet::GetResultBool(int nField)
{
    if ( m_fieldValues[nField-1].IsNull() )
    {
        if ( GetFieldLength(nField) < 0 )
...
wxDateTime wxOdbcResultSet::GetResultDate(int nField)
{
    if ( m_fieldValues[nField-1].IsNull() )
    {
        if ( GetFieldLength(nField) <= 0 )
...
double wxOdbcResultSet::GetResultDouble(int nField)
{
    if ( m_fieldValues[nField-1].IsNull() )
    {
        if ( GetFieldLength(nField) < 0 )
...
wxResultSetMetaData* wxOdbcResultSet::GetMetaData()
{
  wxResultSetMetaData* pMetaData = new wxOdbcResultSetMetaData(m_pInterface, m_pOdbcStatement);
  LogMetaDataForCleanup(pMetaData);
  return pMetaData;
}

void wxOdbcResultSet::InterpretErrorCodes( long nCode, SQLHSTMT stmth_ptr )
{
