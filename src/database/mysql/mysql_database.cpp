...
bool  wxMysqlDatabase::ChangeDatabase(const wxString& database)
{
    const char* dbCstr = database.c_str();
    int res = m_pInterface->GetMysqlSelectDatabase()((MYSQL*)m_pDatabase, dbCstr);
    if(res==0)
        return true;
    else
        return false;
...
{
  // Ultimately, this will probably be a map of Mysql database error code values to wxDatabase values
