#include "include/sqlite3/sqlite_resultset_metatdat.h"
...
// ctor
wxSqliteResultSet::wxSqliteResultSet()
 : wxDatabaseResultSet()
{
  m_pStatement = NULL;
...
wxSqliteResultSet::wxSqliteResultSet(wxSqlitePreparedStatement* pStatement, bool bManageStatement /*= false*/)
 : wxDatabaseResultSet()
{
...
// get field
int wxSqliteResultSet::GetResultInt(int nField)
{
  int nValue = -1;
  if (m_pSqliteStatement == NULL)
    m_pSqliteStatement = m_pStatement->GetLastStatement();
...
long wxSqliteResultSet::GetResultLong(int nField)
{
  long nValue = -1;
  if (m_pSqliteStatement == NULL)
    m_pSqliteStatement = m_pStatement->GetLastStatement();
...
bool wxSqliteResultSet::GetResultBool(int nField)
{
  int nValue = 0;
  if (m_pSqliteStatement == NULL)
    m_pSqliteStatement = m_pStatement->GetLastStatement();
...
wxDateTime wxSqliteResultSet::GetResultDate(int nField)
{
...
double wxSqliteResultSet::GetResultDouble(int nField)
{
  double dblValue = -1;
  if (m_pSqliteStatement == NULL)
    m_pSqliteStatement = m_pStatement->GetLastStatement();
...
void* wxSqliteResultSet::GetResultBlob(int nField, wxMemoryBuffer& Buffer)
{
  int nLength = 0;
  if (m_pSqliteStatement == NULL)
    m_pSqliteStatement = m_pStatement->GetLastStatement();
...
bool wxSqliteResultSet::IsFieldNull(int nField)
{
  if (m_pSqliteStatement == NULL)
    m_pSqliteStatement = m_pStatement->GetLastStatement();
...
int wxSqliteResultSet::LookupField(const wxString& strField)
{
...
wxResultSetMetaData* wxSqliteResultSet::GetMetaData()
{
  wxResultSetMetaData* pMetaData = new wxSqliteResultSetMetaData(m_pSqliteStatement);
